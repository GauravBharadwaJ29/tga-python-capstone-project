...
services:
  ...
  product_catalog:
    ...
    ports:
      - "8001:8001"
    volumes:
      - product_catalog_logs:/var/log/product_catalog_service
  ...
  notification:
    ...
    ports:
      - "8007:8007"
    volumes:
      - notification_service_logs:/var/log/notification
...
volumes:
  ...
  order_service_logs:
    ...
    driver_opts:
      ...
      o: bind
  # --- Added line for Kafka KRaft mode ---
  kafka_data:
  # --- End added line ---
...
```
ORDERS_JSONL=sample_data/orders.jsonl
PRODUCTS_CSV=sample_data/products.csv
STORES_CSV=sample_data/stores.csv
OUTPUT_DIR=output/
...
## File: `analytics\src\analytics_job.py`
...
## File: `analytics\src\logger_config.py`
...
## File: `analytics\src\logging_config.yaml`
...
formatters:
  standard:    
    format: '%(asctime)s - %(levelname)s - [${SERVICE_NAME}] - %(filename)s:%(lineno)d - %(message)s'
...
## File: `api_gateway\requirements.txt`
...
## File: `api_gateway\src\config.py`
...
## File: `api_gateway\src\logger_config.py`
...
## File: `api_gateway\src\logging_config.yaml`
...
## File: `api_gateway\src\routes.py`
...
## File: `api_gateway\src\security.py`
...
## File: `api_gateway\tests\test_routes.py`
...
```
DATABASE_URL=postgresql+asyncpg://postgres:postgres@postgres:5432/psmarket
SERVICE_NAME=auth_service
LOG_DIR=/var/log/auth_service
...
## File: `auth_service\requirements.txt`
...
## File: `auth_service\src\database.py`
...
## File: `auth_service\src\logger_config.py`
...
## File: `auth_service\src\logging_config.yaml`
...
## File: `auth_service\src\models.py`
...
## File: `auth_service\src\routes.py`
...
## File: `auth_service\tests\test_routes.py`
...
```
POSTGRES_URI=postgresql+asyncpg://postgres:postgres@postgres:5432/psmarket
KAFKA_BOOTSTRAP_SERVERS=kafka:9092
BILLING_TOPIC=billing_events
PAYMENT_TOPIC=payment_events
CONSUMER_BILLING_GROUP=billing_group
...
EVENT_INVOICE_FAILED=invoice_failed
EVENT_INVOICE_CANCELLED=invoice_cancelled
PORT=8005
SERVICE_NAME=billing_service
LOG_DIR=/var/log/billing_service
...
## File: `billing\src\kafka_consumer.py`
...
## File: `billing\src\kafka_producer.py`
...
## File: `billing\src\logger_config.py`
...
## File: `billing\src\logging_config.yaml`
...
app = FastAPI(
    title=f"{os.getenv('SERVICE_NAME', 'Billing Service')}",
    description="Generates and manages bills. Publishes events to Kafka.",
    version="1.0.0",
...
#     logger.info(f"Creating new bill for order {bill.order_id}")
#     # Validate total amount matches sum of items
#     total_amount = sum(item.quantity * item.price for item in bill.items)
#     if abs(bill.amount - total_amount) > 0.01:
...
async def list_bills(db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Billing).where(Billing.deleted == False))
    bills = result.scalars().all()
    return bills
...
async def get_bill(bill_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Billing).where(Billing.id == bill_id , Billing.deleted == False))
...
#     bill.deleted = True
#     await db.commit()
#     db.add(BillingAuditLog(billing_id=bill.id, old_status=bill.status, new_status="deleted", changed_by=user ))
#     await db.commit()
#     await kafka.send_event({"event": "bill_deleted", "bill_id": bill_id})
#     return {"message": "Bill soft-deleted"}
...
async def get_bill_by_order(order_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Billing).where(Billing.order_id == order_id, Billing.deleted == False))
    bill = result.scalar_one_or_none()
    if not bill:
        raise HTTPException(status_code=404, detail="Bill not found")
    return bill
...
async def get_bills_by_customer(customer_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Billing).where(Billing.customer_id == customer_id, Billing.deleted == False))
    bills = result.scalars().all()
...
async def list_deleted_bills(db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Billing).where(Billing.deleted == True))
    bills = result.scalars().all()
    return bills
...
async def get_audit_logs(bill_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(BillingAuditLog).where(BillingAuditLog.billing_id == bill_id))
...
## File: `billing\tests\test_routes.py`
...
ORDER_TOPIC=order_events
PAYMENT_TOPIC=payment_events
DELIVERY_TOPIC=delivery_events
NOTIFICATION_TOPIC=notification_events
CONSUMER_DELIVERY_GROUP=delivery_group
EVENT_DELIVERY_CREATED=delivery_created
EVENT_DELIVERY_CANCELLED=delivery_cancelled
EVENT_DELIVERY_UPDATED=delivery_updated
EVENT_DELIVERY_DELETED=delivery_deleted
EVENT_PAYMENT_SUCCESSFUL=payment_successful
EVENT_ORDER_CANCELLED=order_cancelled
...
LOG_DIR=/var/log/delivery_service
...
## File: `delivery\requirements.txt`
...
## File: `delivery\src\exceptions.py`
...
## File: `delivery\src\kafka_consumer.py`
...
## File: `delivery\src\kafka_producer.py`
...
## File: `delivery\src\logger_config.py`
...
## File: `delivery\src\logging_config.yaml`
...
## File: `delivery\tests\test_routes.py`
...
## File: `front_end\package-lock.json`
...
## File: `front_end\public\manifest.json`
...
## File: `front_end\src\App.test.js`
...
## File: `front_end\src\AuthContext.js`
...
## File: `front_end\src\reportWebVitals.js`
...
## File: `front_end\src\setupTests.js`
...
POSTGRES_URI=postgresql+asyncpg://postgres:postgres@postgres:5432/psmarket
KAFKA_BOOTSTRAP_SERVERS=kafka:9092
INVENTORY_TOPIC=inventory_events
ORDER_TOPIC=order_events
CONSUMER_INVENTORY_GROUP=inventory_group
EVENT_ORDER_CREATED=order_created
EVENT_ORDER_FAILED=order_failed
EVENT_PAYMENT_SUCCESSFUL=payment_successful
EVENT_INVENTORY_CREATED=inventory_created
EVENT_INVENTORY_UPDATED=inventory_updated
EVENT_INVENTORY_DELETED=inventory_deleted
PORT=8002
SERVICE_NAME=inventory_service
LOG_DIR=/var/log/inventory_service
...
## File: `inventory\requirements.txt`
...
## File: `inventory\sample_data\products.json`
...
## File: `inventory\src\database.py`
...
## File: `inventory\src\kafka_consumer.py`
...
## File: `inventory\src\kafka_producer.py`
...
## File: `inventory\src\logger_config.py`
...
## File: `inventory\src\logging_config.yaml`
...
## File: `inventory\tests\test_routes.py`
...
LOG_DIR=/var/log/notification

```


## File: `notification\README.md`
...
## File: `notification\requirements.txt`
...
## File: `notification\src\config.py`
...
## File: `notification\src\logger_config.py`
...
## File: `notification\src\logging_config.yaml`
...
## File: `notification\src\models.py`
...
## File: `notification\src\mongo_log.py`
...
## File: `notification\src\redis_queue.py`
...
## File: `notification\src\routes.py`
...
## File: `notification\src\utils.py`
...
## File: `notification\tests\test_routes.py`
...
STORE_TOPIC=store_events
NOTIFICATION_API_URL=http://notification:8007/notify
CONSUMER_BRIDGE_GROUP=notification_group
EVENT_PAYMENT_SUCCESSFUL=payment_successful
EVENT_PAYMENT_FAILED=payment_failed
EVENT_ORDER_SHIPPED=order_shipped
EVENT_ORDER_DELIVERED=order_delivered
LOG_LEVEL=INFO
LOGGING_CONFIG=src/logging_config.yaml
SERVICE_NAME=notification_bridge
LOG_DIR=/var/log/notification_bridge
```


## File: `notification_bridge\README.md`
...
## File: `notification_bridge\requirements.txt`
...
## File: `notification_bridge\src\bridge.py`
...
## File: `notification_bridge\src\config.py`
...
## File: `notification_bridge\src\event_mapper.py`
...
## File: `notification_bridge\src\logger_config.py`
...
## File: `notification_bridge\src\logging_config.yaml`
...
## File: `notification_bridge\src\main.py`
...
## File: `notification_bridge\tests\test_bridge.py`
...
PAYMENT_TOPIC=payment_events
NOTIFICATION_TOPIC=notification_events
CONSUMER_ORDER_GROUP=order_group
EVENT_ORDER_CREATED=order_created
EVENT_ORDER_CANCELLED=order_cancelled
EVENT_ORDER_UPDATED=order_updated
EVENT_ORDER_DELETED=order_deleted
EVENT_ORDER_FAILED=order_failed
EVENT_ORDER_FAILED_NOTIFICATION=order_failed_notification
EVENT_PAYMENT_FAILED=payment_failed
EVENT_PAYMENT_SUCCESSFUL=payment_successful
PORT=8003
SERVICE_NAME=order_service
LOG_DIR=/var/log/order_service
MAX_RETRY_ATTEMPTS=10
# Add to .env for more configuration options
SQL_DEBUG=false
DB_POOL_SIZE=5
DB_MAX_OVERFLOW=10
...
from src.models import Base  # Adjust the import path if needed
...
## File: `order\src\kafka_consumer.py`
...
## File: `order\src\kafka_producer.py`
...
## File: `order\src\logger_config.py`
...
## File: `order\src\logging_config.yaml`
...
ORDER_TOPIC=order_events
NOTIFICATION_TOPIC=notification_events
CONSUMER_PAYMENT_GROUP=payment_group
EVENT_ORDER_CREATED=order_created
EVENT_ORDER_CANCELLED=order_cancelled
EVENT_PAYMENT_SUCCESSFUL=payment_successful
EVENT_PAYMENT_FAILED=payment_failed
...
EVENT_PAYMENT_UPDATED=payment_updated
EVENT_PAYMENT_DELETED=payment_deleted
PORT=8004
SERVICE_NAME=payment_service
LOG_DIR=/var/log/payment_service
KAFKA_RETRY_COUNT=10
KAFKA_RETRY_DELAY=5
SQL_DEBUG=false
```


## File: `payment\README.md`
...
## File: `payment\src\kafka_consumer.py`
...
## File: `payment\src\kafka_producer.py`
...
## File: `payment\src\logger_config.py`
...
## File: `payment\src\logging_config.yaml`
...
app = FastAPI(
    ...
    description="Processes payments (dummy logic). Publishes events to Kafka.",
    version="1.0.0",
    lifespan=lifespan
)
...
## File: `payment\tests\test_routes.py`
...
PORT=8001
SERVICE_NAME=product_catalog_service
LOG_DIR=/var/log/product_catalog_service
KAFKA_RETRY_COUNT=10
KAFKA_RETRY_DELAY=5
...
## File: `product_catalog\README.md`
...
## File: `product_catalog\requirements.txt`
...
## File: `product_catalog\sample_data\products.json`
...
## File: `product_catalog\src\config.py`
...
## File: `product_catalog\src\database.py`
...
## File: `product_catalog\src\kafka_producer.py`
...
## File: `product_catalog\src\logger_config.py`
...
## File: `product_catalog\src\logging_config.yaml`
...
## File: `product_catalog\src\main.py`
...
## File: `product_catalog\src\models.py`
...
## File: `product_catalog\src\routes.py`
...
## File: `product_catalog\src\utils.py`
...
## File: `product_catalog\tests\test_routes.py`
...
KAFKA_BOOTSTRAP_SERVERS=kafka:9092
STORE_TOPIC=store_events
EVENT_PRODUCT_ONBOARDED=product_onboarded
EVENT_STORE_ONBOARDED=store_onboarded
EVENT_STORE_UPDATED=store_updated
EVENT_STORE_DELETED=store_deleted
PORT=8008
SERVICE_NAME=store_onboarding_service
LOG_DIR=/var/log/store_onboarding_service
KAFKA_RETRY_COUNT=10
KAFKA_RETRY_DELAY=5
SQL_DEBUG=false
...
## File: `store_onboarding\README.md`
...
## File: `store_onboarding\requirements.txt`
...
## File: `store_onboarding\src\config.py`
...
## File: `store_onboarding\src\database.py`
...
## File: `store_onboarding\src\kafka_producer.py`
...
from aiokafka import AIOKafkaProducer
from .config import (
    KAFKA_BOOTSTRAP_SERVERS, STORE_TOPIC, 
    KAFKA_RETRY_COUNT, KAFKA_RETRY_DELAY,
    ErrorMessages, SERVICE_NAME as service_name
)
from .logger_config import logger
...
class KafkaProducer:
    ...
    async def send_event(self, event: dict):
        try:
            ...
            await self.producer.send_and_wait(STORE_TOPIC, event)
            logger.debug(f"Event sent to topic {STORE_TOPIC}")
        except Exception as e:
            logger.error(f"Failed to send event: {str(e)}")
            raise RuntimeError(ErrorMessages.KAFKA_EVENT_FAILED.format(str(e)))
...
## File: `store_onboarding\src\logger_config.py`
...
## File: `store_onboarding\src\logging_config.yaml`
...
## File: `store_onboarding\src\main.py`
...
## File: `store_onboarding\src\models.py`
...
## File: `store_onboarding\src\routes.py`
...
## File: `store_onboarding\src\utils.py`
...
## File: `store_onboarding\tests\test_routes.py`
